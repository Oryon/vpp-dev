  ...
  if (dev_class->rx_mode_change_function)
    if ((err =
	 dev_class->rx_mode_change_function (vnm, hw_if_index, queue_id,
					     disable ?
					     VNET_HW_INTERFACE_RX_MODE_DISABLED
					     : rxq->rx_mode)) != NULL)
			       ...
			       u16 queue_id, vnet_hw_interface_rx_mode * mode)
{
  ...
  if (!rxq || !rxq->current_rx_queue_runtime)
    {
...
int
vnet_hw_interface_set_rx_rss_mask (vnet_main_t * vnm, u32 hw_if_index,
				   u32 thread_index, u16 rss_mask)
{
  ...
  if (rss_mask < rt->rss_mask)
    {
      /* We have to change rss slot for queues which have a too high rss slot */
      u16 rss_slot;
      for (rss_slot = rss_mask + 1; rss_slot < rt->rss_mask + 1; rss_slot++)
	while (vec_len (rt->queues_per_rss[rss_slot]))
	  {
	    vnet_hw_interface_set_rx_thread (vnm,
					     rt->
					     queues_per_rss[0]->hw_if_index,
					     rt->queues_per_rss[0]->queue_id,
					     thread_index,
					     rss_slot & rss_mask);
	  }
				    ...
				    vnet_hw_interface_t * hw)
{
  ...
  for (thread_index = 0;
	...
	if (queue->tx_queue_index != VNET_HW_INVALID_QUEUE &&
	    hw->tx_queues[queue->tx_queue_index].thread_index == ~0)
	  {
	    queue->tx_lock = &hw->tx_queues[queue->tx_queue_index].lock;
	  }
      }
    }
